...
| F2-test-gap | major | The F2 regression test admitted it could not reproduce a purge and only checked contiguous/beyond-latest, yet the checklist marked F2's purged detection ✓. | Added `test_wal_get_updates_since_purged_detected`: flushes the memtable and deletes archived WAL files (`flush_and_purge_wal_for_test()`), then asserts `get_updates_since(0)` returns `ERR_LSN_PURGED`. Hardened per the 3rd review: `flush_and_purge_wal_for_test()` now probes `GetUpdatesSince(1)` and returns whether a purge was actually confirmed; when it was, the test **hard-asserts** `ERR_LSN_PURGED` (it can no longer pass vacuously by taking the contiguous branch). |
...
| 2026-07-03 | Third review of `8253147` verified all second-review fixes; hardened the F2 test to hard-assert `ERR_LSN_PURGED` when a purge is confirmed (its only remaining PARTIALLY-FIXED note). |
