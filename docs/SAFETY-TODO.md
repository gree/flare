...
| SAF-10d | Acceptance suite passes 12/12 on one revision (runs 7 and 8; the suite: T1 initial copy under load, T2/T3 cut+heal, T5 repeated cuts, T8 operator restart, T14/T16 bulk operation and failover epoch change, read eligibility, ledger ownership, stats-fetch failure); the non-WAL regression suites pass 30/30 on the same operator. The E2E found four defects, all fixed on the branch (flared follower thread lifecycle 325a2f8; operator drop observation hidden by an unreadable replica eb2ba17; tch follower reading held as Unknown and 3 s-per-probe cost 2379cff). T4 (replica crash with a backlog, PVC-backed) staged in run 10 (13/13) and exposed a pre-existing count defect (curr_items estimate blind to WAL-only keys after a reopen; fixed in flared). T6 (retention purge → lsn_purged → rebuild) and a bounded T9 (15 MB backlog, RSS, positional tombstone GC) staged 2026-09-21 in their own suites and passing on CI run 3 (ecb1c40); T6 found and fixed a fifth defect (RocksDB serves the oldest surviving WAL file instead of NotFound; served gap → lsn_purged, 3b7f38c). Still not staged: T17 lock-hold measurement (deferred by agreement); T10-T13/T15 unit-pinned. Scale evaluation: follow stream ≈255 changes/poll (bounded by repl-follow-max-batches), boot scan ≈1.07 µs/key at 0.8M keys, master OOM at 512Mi under a 2M-key load (harness budget raised to 2Gi; rerun pending). PR #144 open (draft; pull_request CI runs are not created for the stacked PR, evidence via workflow_dispatch). Author-run only, nothing verified. Reports: docs/reports/2026-09-20-saf10d-minimal-e2e-continuous-replication.txt |
