...
| <a id="ev-03"></a>EV-03 / SC-03 | A replica with missed writes remains repair-pending until reconstruction completes and missing data is recovered. | H3, H2, UCA-02, UCA-32 | partial | unverified | [code](../flare_operator/FlareOperator/StateMachine/ReplicaRepair.lean), [check](../flare_operator/FlareOperator/E2E/Tests/ReplicaRepair.lean) |
| <a id="ev-04"></a>EV-04 / SC-04 | Normal promotion and Prepare activation require a completed usable copy associated with the current source. | H2, H4, UCA-10, UCA-26 | partial | unverified | [code](../flare_operator/FlareOperator/StateMachine/SyncEvidence.lean), [check](../flare_operator/FlareOperator/E2E/Tests/PrepareEvidence.lean) |
| <a id="ev-05"></a>EV-05 / SC-05 | Delete only a revalidated target when independent current evidence supports the surviving copy. | H5, H2, UCA-07, UCA-11, UCA-12, UCA-13, UCA-26 | partial | unverified | [code](../flare_operator/FlareOperator/StateMachine/StatsObservation.lean), [check](../flare_operator/FlareOperator/StateMachine/StatsObservation.lean) |
...
| <a id="ev-13"></a>EV-13 / SC-13 | A future replication design must specify what acknowledged writes survive and how replicas catch up after interruption. | H2, H3, H5, UCA-02, UCA-27 | partial | unverified | [code](../src/lib/queue_proxy_write.cc), [check](../flare_operator/FlareOperator/E2E/Tests/StrictDurability.lean) |
...
| <a id="ev-16"></a>EV-16 / SC-16 | Continuous fetch resumes without external intervention, and a lost connection alone never triggers a rebuild. | H3, H4, UCA-19, UCA-32 | partial | unverified | [code](../src/lib/handler_wal_follower.cc), [check](../test/lib/test_storage_rocksdb.cc) |
| <a id="ev-17"></a>EV-17 / SC-17 | The applied position is contiguous and crash-atomic, and no other path may advance it. | H2, H3, UCA-20, UCA-23 | implemented | unverified | [code](../src/lib/storage_rocksdb.cc), [check](../test/lib/test_storage_rocksdb.cc) |
| <a id="ev-18"></a>EV-18 / SC-18 | History loss is an explicit rebuild state. | H2, H3, H5, UCA-24, UCA-25, UCA-33 | implemented | unverified | [code](../src/lib/handler_wal_follower.cc), [check](../test/lib/test_storage_rocksdb.cc) |
| <a id="ev-19"></a>EV-19 / SC-19 | Both delivery paths apply through one comparison rule; no path may reorder, duplicate or resurrect, and every path that can change a key participates in the label capture. | H2, H3, H6, UCA-21, UCA-22, UCA-24, UCA-29, UCA-30, UCA-31 | partial | unverified | [code](../src/lib/storage_rocksdb.cc), [check](../test/lib/test_storage_rocksdb.cc) |
| <a id="ev-20"></a>EV-20 / SC-20 | Delete history is discarded only when the applied position makes an older delivery impossible, and the decision is serialized with the write. | H2, H3, UCA-22, UCA-28 | implemented | unverified | [code](../src/lib/storage_rocksdb.cc), [check](../test/lib/test_storage_rocksdb.cc) |
...
A2 = Pod deletion; A4 = Service/endpoint membership;
A5 = master WAL production and retention (flared); A6 = replica continuous
fetch/apply controller (flared, SAF-10: connect / fetch / decide / apply /
reconnect / declare-rebuild; feedback F7 = applied position, stream state,
last progress, reason, skip counters — consumed by A1/A2/A4). Stable UCA IDs map
...
| UCA-19 | A6 | Not provided | Fetch is not resumed after a transient disconnection | H3 | [EV-16](#ev-16) |
| UCA-20 | A6 | Provided | The applied position advances past a range that was not applied (e.g. a forwarded write taken as WAL progress) | H2, H3 | [EV-17](#ev-17) |
| UCA-21 | A6 | Wrong order | A newer value arrives by forwarding and an older WAL change is then applied over it | H2, H3 | [EV-19](#ev-19) |
| UCA-22 | A6 | Wrong order | A delete is applied and an older put then resurrects the key | H2, H3 | [EV-19](#ev-19), [EV-20](#ev-20) |
| UCA-23 | A6 | Provided | The process crashes after applying changes and before the position is saved, so the same range is re-applied or skipped inconsistently | H2, H3 | [EV-17](#ev-17) |
| UCA-24 | A6 | Provided | A batch from an old session or lineage is applied | H2, H3, H6 | [EV-18](#ev-18), [EV-19](#ev-19) |
| UCA-25 | A6 | Provided | A replica whose required history is gone is treated as synchronised | H2, H3 | [EV-18](#ev-18) |
| UCA-26 | A1b | Provided | A lagging or unobservable replica is treated as safe for reads, promotion or deletion | H2, H4, H5 | [EV-04](#ev-04), [EV-05](#ev-05) |
| UCA-27 | A5 | Not provided | WAL is retained until the master's disk is exhausted | H1, H5 | [EV-13](#ev-13) |
| UCA-28 | A6 | Too early | Delete history is discarded while a change older than the delete can still be applied | H2, H3 | [EV-20](#ev-20) |
| UCA-29 | A6 | Wrong order | The apply decision is made against a position read before a stall, retry or lock wait, so a change admitted against an old cursor is written after the cursor moved | H2, H3 | [EV-19](#ev-19) |
| UCA-30 | A6 | Provided | After a rebuild, changes from the previous session, or metadata inherited from the source, are applied | H2, H3, H6 | [EV-19](#ev-19) |
| UCA-31 | A6 | Provided | A key is changed by a path that does not capture an order label (bulk operation, internal delete), so the change cannot be ordered against the other delivery path | H2, H3 | [EV-19](#ev-19) |
| UCA-32 | A6 | Provided | A transient disconnection is treated as a repair trigger and starts a full reconstruction | H3, H4 | [EV-03](#ev-03), [EV-16](#ev-16) |
| UCA-33 | A6 | Provided | A partially initialised copy (metadata cleared, cursor or generation not yet written) accepts deliveries or is presented as a copy | H2, H5 | [EV-18](#ev-18) |
...
| G3 | WAL sync runs **only** at reconstruction and cluster-replication initial transfer — it is not a live replication log, and the master's WAL retains only `walSizeLimitMb` / `walTtlSeconds` | [EV-13](#ev-13), [EV-16](#ev-16), [EV-18](#ev-18) | SAF-10 (branch, unverified): a continuous WAL follower on RocksDB (`repl-follow-enabled`) resumes from its applied position after a disconnection and declares `needs_rebuild` when the history is gone; see docs/design-continuous-wal-replication.md | Continuous WAL shipping so a replica resumes from its cursor after a blip. This is the root fix for G1 as well, and a large redesign: the sync-write contract, replica reads, cluster replication and migration all assume proxy semantics |
