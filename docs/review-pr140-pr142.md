...
## 9. 形式手法の有効性評価(レビューワー向け・2026-07-14)

「Lean/形式モデルは本当に信頼性に寄与したのか」への評価。承認判断の材料として、誇張も過小評価もせず記録する。

### 9.1 実バグ8件の層別分布(本開発で発見・修正した全件)

| 層 | バグ | 件数 |
|---|---|---|
| **証明済み純粋層**(言明済みの性質: master 高々1) | — | **0** |
| 純粋層だが**未言明の性質** | 幽霊蘇生(鮮度・順序)、ゾンビ昇格(生存性) | 2 |
| **モデル外**(IO 層・C++) | iter_begin ロック、truncate 排他、SIGHUP タイミング、WAL 未到達、LSN 鶏卵、truncate 最終コピー | 6 |

**バグは 100% 「分析していない場所」か「言明していない性質」に発生した**。これは seL4 / CompCert で観測された分布(検証済み中心部からはバグが出ず、未検証周辺に集中)と同型であり、「形式手法が効かない」ではなく「仕様カバレッジの問題」を示す。

### 9.2 「作られなかったバグ」の証拠(反実仮想)

- **二重 master / split-brain は全開発期間・全 CI ラウンドで一度も発生していない**。障害注入・ロール再割当が激しく荒れた条件下での実績であり、初期の redis/etcd operator が踏んだ古典的バグ族が「作られなかった」ことを示唆する。不変条件が仕様として先にあり、割当ロジックが仕様補題(`findPartitionNeedingMaster_spec`)込みで書かれた効果
- FSM の measure 設計により「終わらない reconcile」「忘れられた状態」が構造的に排除された(唯一のハングは**モデル外**の kubectl stdin)

### 9.3 モデル(TLA+ 的分析)の実働実績

- **ゾンビ化は実環境より先にモデルが再現した**(scenario4 の `#eval`、数秒)— CI 1周70分に対する診断速度の差
- 幽霊蘇生(scenario5)も同様。修正(`regEpoch`)はモデル上で設計・検証してから実装した
- 発見されたバグは全件、decide 定理として**コンパイル時回帰テスト化**されている(再発 = ビルド失敗)

### 9.4 公平な比較(技術選定の評価)

| 選択肢 | 得られたもの | 失うもの |
|---|---|---|
| **Lean(採用)** | モデル・回帰定理・一般帰納証明が同一言語/定義で接続 | エコシステム(informer なし・5秒ポーリング)、保守人材の希少性 |
| TLA+ + Go | モデル検査(9.3 相当)は同等に可能 | 一般帰納証明(無制限トレース)は実質不可。スペックと実装の乖離リスク(今回それをモデル接続で潰した経験がまさに示す) |
| Go + テストのみ | 開発・保守効率 | 9.2 の「作られなかったバグ」を作っていた可能性。回帰固定が弱い |

**結論**: 発見の主役は E2E + 診断ログ、予防と回帰固定の主役は形式モデル。教訓は「分析を減らす」ではなく「**仕様を広げる**」— 次節の通り、未言明だったデータ保全不変条件をモデルに追加済み。
