...
op* op_parser_text_node::_determine_op(const char* first, const char* buf, int& consume) {
	...
	if (strcmp(first, "get") == 0) {
		...
		r = static_cast<op*>(_new_ op_decr(this->_connection, singleton<flared>::instance().get_cluster(), singleton<flared>::instance().get_storage())); 
		...
		r = static_cast<op*>(_new_ op_show_node(this->_connection));
	} else {
		r = static_cast<op*>(_new_ op_error(this->_connection)); 
	}
