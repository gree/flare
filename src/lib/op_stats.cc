...
int op_stats::_send_stats(thread_pool* tp, storage* st, cluster* cl) {
	...
	_send_stat("flare_node_status", status_object->get_status_code() == status::status_ok ? "OK" : "NG");
	_send_stat("flare_node_map_version" , cl->get_node_map_version());
...
int op_stats::_parse_text_client_parameters(stats_results& results) {
	for (;;) {
		...
		results[match.str(1)] = match.str(2);
